  ...
  return (
    <>  
        <div className='container' style={{color: props.mode === 'dark' ? 'white' : 'black'}}>
          <h2>{props.label}</h2>
          <div className="mb-3">
              <textarea placeholder='Enter Text Here' className="form-control" id="myBox" rows="8" value={text} onChange={ (e) => setText(e.target.value)} style={{backgroundColor: props.mode === 'dark' ? 'grey' : 'white', color: props.mode === 'dark' ? 'white' : 'black'}}></textarea>
          </div>
          <button className="btn btn-primary " onClick={clearTextClick.bind(this)}>Clear Text</button>
          <button className="btn btn-primary mx-3" onClick={toUpperCaseClick.bind(this)}>To Upper Case</button>
          ...
          <button className="btn btn-primary" onClick={handleExtraSpaces.bind(this)}>Remove Extra Space</button>
        </div>
